 ...
 * Google 翻译在本网络不可达，不用。
 *
 * 实测规则：单次 ≤~1000 字（超出报 103）；新内容约每分钟 5 条（超出报 411）；
 * 重复内容走缓存不计数。
 *
 * 方向自动判断：中文为主 → 译成英文；其他 → 译成中文。
 * options.interactive=true 用于划词/圈译这类单发交互：快速失败不干等。
 */
  ...
  try {
    ...
    if (attempt < maxAttempt && (code === '411' || code === '103')) {
      if (code === '103' && q.length > 500) {
        ...
        const a = await translateChunk(q.slice(0, half), to, options, attempt + 1);
        const b = await translateChunk(q.slice(half), to, options, attempt + 1);
        return `${a}\n${b}`;
      }
 ...
 * options.onProgress(done, total) 用于长文本进度；options.interactive 快速失败。
 */
async function translate(text, options = {}) {
  ...
  try {
    for (let i = 0; i < chunks.length; i += 1) {
      parts.push(await translateChunk(chunks[i], to, options));
      options.onProgress?.(i + 1, chunks.length);
      if (i < chunks.length - 1) await new Promise((r) => setTimeout(r, options.interactive ? 300 : 12000));
    }
  } catch (err) {
