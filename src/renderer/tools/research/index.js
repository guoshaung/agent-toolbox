...
/**
 * 科研门户：把常用的科研/AI 网站收进一个格子铺，点开就是常驻的内嵌页面。
 * 每个站点一个独立 webview，登录态和页面状态都保住，切换是瞬时的。
 * favicon 由主进程代取（渲染进程 CSP 只放行 self/data:），拿不到就用 emoji 兜底。
 */
export default {
  id: 'research',
  title: '科研',
  icon: '🔬',
  hint: '科研与 AI 网站格子铺：玻尔 / 掌桥 / 纳米 / 当贝 / 星火，可自己加',
  ...
  create(root, ctx) {
    ...
    const viewBar = h('div', { class: 'bar bar--drag', hidden: true },
    ...
    async function faviconFor(url, emoji) {
      if (faviconCache.has(url)) return faviconCache.get(url);
      const cached = config.get(`research.favicons.${btoa(url).slice(0, 32)}`);
      if (cached) {
      ...
      if (dataUrl) config.set(`research.favicons.${btoa(url).slice(0, 32)}`, dataUrl);
      return dataUrl;
    }

    function siteIcon(site) {
      const icon = h('span', { class: 'research__icon' }, site.emoji || '🌐');
      faviconFor(site.url, site.emoji).then((dataUrl) => {
        if (dataUrl) {
          icon.textContent = '';
    ...
    const mainBar = h('div', { class: 'bar bar--drag research__bar' },
      h('strong', {}, '科研门户'),
      h('span', { class: 'faint research__hint' }, '点开就是常驻页面，登录态各自保留'),
    );
    ...
    function showGrid() {
      ...
      mainBar.removeAttribute('hidden');
    ...
    root.append(
      mainBar,
      viewBar,
      h('div', { class: 'research__body' }, grid, viewHost),
    );
