...
可额外提供右侧图（鼻尖朝右，独立运行时命名 `input-right.png`）。四向贴图现在覆盖两个侧面；没有右侧图时，右侧贴图使用左侧镜像，`texture-provenance.json` 会记录来源。镜像只能补齐投影，不能还原非对称衣服或改善原有侧面几何。

新任务会把裁剪后的高分辨率源保存为 `texture-source-*.png`，1024 方图仅用于 Hunyuan 形状条件；最终投影优先使用高分辨率源。可见脸部区域锁定正面参考，避免侧面 AI 参考把眼睛重复贴到头部侧面。
...
这是自动重建草稿，不是精修角色。单图背面和遮挡部分由模型猜测，脸/手细节会损失，可能有对称/背面伪影。绑定为启发式初始估计，保留原图姿势；动作重定向前需要人工校正 T-pose 和权重。默认绑定没有手指骨骼；没有口型、表情 BlendShape、头发/布料物理。实验性 `--t-pose` 可能拉坏连在身体上的衣服和头发，默认禁用。

## 可选五指替换与验收

对于粘连的手部，单纯增加骨骼不能生成五指。可在备份原任务后，为新任务提供人工核对的 `hand-repair.json`，再运行 `export_vrm.py --job jobs/<id>`。当前仅支持近正面的手掌和米制关节坐标，必须逐模型确认切除范围，不能把样例坐标套到所有人物上。

配置包含 `hands.left` / `hands.right`；每侧含三维 `wrist`、非零 `direction`、`width` 和 `palmLength`（后两项单位米，范围 0.02–0.07）。可选 `skinColor` 为 RGBA。修复会在有界范围内替换粘连手部，生成通用手掌与五指，并设置每手 15 根有权重的手指骨骼和 5 个末端辅助节点。总计 51 根 VRM 人形骨骼、10 个辅助节点。它不是从图片精确恢复的手，也未自动修复手腕接缝和身体绑定。

查看器可切左右手近景、显示骨骼、单指弯曲和复位。`hand-rig.json` 记录权重覆盖和测试轴；未提供修复配置时维持原来的 21 骨骼导出，并清除过期手指测试元数据。原始 `mesh.npz` 不被导出器修改，移除配置后可以重新导出原手。替换手不支持同时启用实验性 `--t-pose`。

## 面部表情

VRM 表情必须有面部 Morph Target，不能只靠头骨旋转。可选的 `face-expressions.json`
需要人工核对左右眼和嘴部的三维区域；导出器会生成 `blinkLeft`、`blinkRight`、
`happy` 和 `aa` 四个标准预设，查看器才会显示表情控制。它们是保守的标注驱动形变，
不是从单张图片恢复的真实面部肌肉，也不能修复本身已经重复或错位的眼睛。没有该配置时，
VRM 明确不声明表情能力。

可选的 `face-shape.json` 可为经人工检查的鼻部给出中心、范围和最大前伸量，避免把写实重建的长鼻梁保留下来。它只收回受标记范围内的前向尖点，不会猜测或重建缺失的头顶、发型或侧面几何。

查看器的“走路”和“挥手”用于验收蒙皮：走路使用低幅度反向腿摆；挥手限定在明确加权的手部，避免粗糙袖子权重被大幅旋转时撕裂。动作控件不会把当前草稿等同于可用于完整舞蹈、动捕或游戏的绑定。

可选 `torso-cloth.json` 会复制经人工确认的前躯干外衣表面，偏移为独立的外层，并导出 `chestBounce` 自定义 VRM Morph。查看器只在走路时给它 0.18–0.74 的小幅滞后权重。它提供外衣与底层表面的最小分层，不是完整的人体、服装碰撞或软体模拟；不应将示例范围套到其它角色。
