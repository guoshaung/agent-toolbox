...
DSH Web 会在工具箱启动时后台检测 `dsh web`：已有 `3080` 端口实例时直接复用，没有时会尝试在工具箱运行目录下载 `@deepseek-ai/dsh` 并启动。顶部横幅是进入内置 Harness 的大入口；首次自动下载需要本机已有 Node.js/npm。
...
| ◈ DSH | 后台复用或启动 `dsh web`，从顶部横幅进入 DeepSeek Harness 内置 Web 控制台 | — |
| ◉ Voicebox | 官方开源 Voicebox 启动壳：检测本机安装、打开原生应用、跳转 GitHub / 下载页 | — |
| ◉ 数字人 | 论文讲解台：导入科研 PPT 讲稿、逐句字幕、系统语音和显式 AI 精修 | — |
...
左侧「手机」默认关闭。点击「启动手机控制」后，让手机和电脑连接同一个 Wi-Fi，工具内会显示当前有效的 APK 下载二维码和配对地址。手机端调用 AI 时仍复用电脑端当前选择的 DeepSeek 网页版或第三方兼容 API，因此切换 AI 供应商不会破坏远程功能。配对令牌会加密保存，应用重启后可以继续使用；「重新配对」会让旧地址立即失效。勾选「开机自动启动」后，工具箱启动时会自动打开远程服务。Android 浏览器打开配对页后也可安装到主屏幕；安装后的 PWA 支持系统分享目标，论文、视频、公众号链接或文字可以直接分享到 Agent 收件箱，并同步显示在电脑端「手机」页面。
...
「Voicebox」在左侧固定入口中，是 `jamiepine/voicebox` 官方开源项目的启动壳，不是工具箱自制的语音应用。工具箱只检测本机是否安装 Voicebox：已安装时用系统原生方式打开 `Voicebox.app` / `Voicebox.exe`，因此官方图标、界面、模型和权限都不会被替换；未安装时可直接跳转官方下载安装页，也可打开 GitHub 项目和文档。

第一次使用按这个顺序操作：

1. 打开左侧「Voicebox」，点击“下载官方版本”，从 [voicebox.sh](https://voicebox.sh/download) 获取对应系统的安装包。
2. 安装官方应用后回到工具箱，入口会显示“已检测到”。
3. 点击“启动官方 Voicebox”，工具箱会打开官方原生应用本体。

视频报告还可以复用已启动的 Voicebox 本地 REST API：在「视频 → 视频报告」拖入本地视频后，Voicebox 会优先通过 `/transcribe` 做本地 Whisper 转写；打开「Voicebox 朗读摘要」并选择音色后，AI 摘要会提交到 `/generate`，完成后可直接播放。视频原文和音频不会发到云端；Voicebox 不运行、没有音色或生成失败时，报告仍会正常保存。首次加载模型、转写和克隆音色的第一次生成会比较慢，后续通常会快一些；批量任务按顺序生成，避免占满 GPU。

### 数字人

「数字人」是本地动态矢量科研讲解台，不伪装成云端视频生成服务：可以切换晶石研究员、学术导师、实验搭档和实验机器人四种形象，待机时呼吸/眨眼，朗读时同步口型、点头和光环；还可以从「科研 → PPT演示」导入讲稿，逐句高亮字幕，用系统 `SpeechSynthesis` 朗读，也可以显式让当前 AI 精修口语表达。默认文字和朗读留在本机；AI 精修前会显示动作，发送前请核对论文数字、引用和结论。
...
阅读器会按文献保存 PDF/TXT/Markdown 的滚动进度；重新打开时恢复位置，首次打开自动标记为“阅读中”，也可以在阅读器标题栏一键标记“已读”。PDF 阅读器支持当前页/总页数、直接跳页和最近阅读列表；批注栏支持搜索本篇的原文、译文、备注和标签，带有页面锚点的批注可以一键跳回原文。科研功能的后续优先级见 [docs/RESEARCH-ROADMAP.md](docs/RESEARCH-ROADMAP.md)。

对于知网或学校图书馆：点「打开登录页」进入已登录的检索结果页，再点「扫描当前列表」，勾选论文后使用「下载已选」。工具会在当前站点内逐篇打开论文并点击下载入口，最多处理 30 篇；单篇失败会记录原因并继续后面的论文，下载完成自动归档到文献库。批量运行中可点「暂停下载」，已完成项目会取消勾选，剩余项目可以继续；也可以把本轮 JSON 结果导出留档。遇到验证码、人工验证、权限确认或站点结构变化会提示，不会尝试绕过。
...
科研“PPT演示”是独立的多页演示工作台：可以调整页面顺序、复制/删除页面，编辑标题、要点、讲稿和引用，添加一张本地图片，并导出可继续编辑的 `.pptx`，同时可保存 JSON 源文件和 Markdown 大纲。它与单页“PPT图板”分工明确，适合把科研图、文献结论和汇报讲稿组织成一套演示。
...
1. 在工具箱主界面点击菱形的“虚拟形象”入口，按需选择摄像头、`.vrm` 文件和背景。模型路径仅保存在本机配置中，不会复制到仓库。
2. 在 OBS 的“来源”区域点击 `+`，选择“窗口采集”，把窗口设为 `Avatar Output`。
3. 需要透明背景时，在虚拟形象窗口选择“透明”，并在 OBS 的窗口采集属性中启用支持透明度的捕获方式。若当前系统或捕获方式不保留 Alpha，请改用纯色背景并添加“色度键”滤镜。
4. 调整画布和窗口尺寸，点击左上角齿轮隐藏设置面板后开始录制。窗口可置顶，也可切换为无边框模式；这些选项会自动保存。

要把 OBS 画面用于会议软件，在 OBS 中点击“启动虚拟摄像机”，然后到 Zoom、腾讯会议、飞书会议等软件的视频设置里选择 `OBS Virtual Camera`。

摄像头被其他软件独占时，请先关闭占用它的软件，再回到虚拟形象窗口点击“授权并刷新摄像头”。OBS 虚拟摄像机只传视频，麦克风仍在会议软件中单独选择。
