...
| ☀️ 今天 | 首页：刚建的东西、该归位的数量、这周在写的仓库、讲过的项目、没做完的任务、最近用的工具 | 启动默认 |
| 📦 收纳 | 桌面 / 下载 / 主目录顶层散落的东西按类型一键归位（可撤销，散落太多每天提醒一次，可选「下载完问我要不要归位」、「每周日自动把明显的归位」，撤销栈留最近 5 批）；认不出的亮出内容自己挑去处；结构一览图；「刚建的在哪」；拖个项目进来让 AI 讲它是什么、怎么跑、从哪读起，能接着问、一站一站「带我读」、「考考我」五道题、「学习顺序」一键变任务、「帮我写 README」；贴个 GitHub 地址直接拉下来看懂；下载了两遍的一键清 | — |
| 🎨 皮肤 | 23 套主题（镀金、玫瑰金、钛银、深海、森林、落日、熔岩、北欧、银河、琥珀终端、摩卡、赛博朋克、初音、夜樱、白纸、墨水屏、冰川、薄荷、薰衣草…）× 5 种质感效果；可跟随系统深浅色，白天一套晚上一套 | — |
| 🧠 内心独白 | 选中一句话拆出对方真正想说什么；或贴到微信上，卡片跟着每条消息走 | ⌘⇧M |
| 🔌 接口台 / 🌿 Git / 📡 抓包台 | 发 HTTP 请求看响应（curl 互转）、常用 git 组合一键跑、看应用内和本机代理的流量 | — |
| ✋ 手势识别 | 摄像头识手势：打响指弹出应用切换栏，比数字切应用 | — |
...
| 按键 | 干什么 |
|---|---|
| `⌘K` | 命令面板：搜工具、换皮肤、找最近新建的文件夹、在 ~/收纳 里按名字找、看懂某个项目、搜学习记录、剪贴板历史（最近 30 条，只在内存）；「+ 事情」记任务；贴 GitHub 地址拉下来看懂 |
| `⌘⇧L` | 在任何应用里选中一段代码 / 一句话，桌宠弹出四行解释 |
| `⌘⇧A` | 把工具箱主窗口叫回来（关掉窗口后应用还在菜单栏） |
| `⌘⇧M` / `⌘⇧E` | 选中文字 → 内心独白 / 术语解释 |
| `Ctrl+Tab` | 最近使用顺序切工具，按住看、松开切 |
| 拖任何文件到窗口 | 问你想干嘛：看懂项目 / 收进文献 / 收进容器 / 递给手机 |
