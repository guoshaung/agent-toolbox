...
- Add an opt-in, landmark-driven VRM expression path. Reviewed face regions export
  ...
  a test slider.
- Move configured eyelid/mouth geometry from behind the textured face to the front
...
- Nine Python tests: image background preservation, normalization, legacy export,
...
- Eleven targeted Node tests: optional fourth input validation and persistence,
  required inputs, job isolation/failures, setup and safe seed updates.
- Local `fubuki-side-hands-v3`: 137,739 vertices, 275,036 triangles, 61 joints,
  51 humanoid mappings and six material primitives, loaded with three-vrm.
...
- With the reviewed Fubuki face config, three-vrm recognizes all four expression
  presets and applies each selected morph across all six skinned primitives. The
  file contains 92 expression-feature vertices; the exported target buffers
  validate with zero glTF errors/warnings.
- Browser samples confirm a visible eyelid and mouth at full Morph weight. A
  conservative walk produces opposite upper-leg quaternions on consecutive frames.
...
the heuristic body rig still require editing. No automatic landmark detection is
included. The expression morphs are generic
